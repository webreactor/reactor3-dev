...
function getList($page,$per_page,$where='')
{
	$all=0;
	$where_rez=' where 1';
	if($this->fkey!='')$where_rez.=' and `'.$this->fkey.'`="'.$this->fkey_value.'"';
	if($where!='')$where_rez.=' and '.$where;
	if($this->order!='')$where_rez.=' order by '.$this->order;
	$data=$this->_db->pages('select * from '.$this->table.$where_rez,$page,$per_page,$all,$total_rows_count);
...
function getSelect($row,$filter,$forceOne=0,$forceFkey=1)
{
	...
	if($this->fkey!=''&&$forceFkey==1)$where_rez.=' and `'.$this->fkey.'`="'.$this->fkey_value.'"';
