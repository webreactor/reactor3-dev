...
{
	if(constant(strtoupper($module.'_'.$name)) == $value) return;
	global $_db;
	$_db->sql('update '.T_REACTOR_CONFIG.' c,'.T_REACTOR_MODULE.' m set c.`value`="'.$value.'" where c.fk_module=m.pk_module and m.`name`="'.$module.'" and c.`name`="'.$name.'"');
...
function reactor_ini_set_array($array)
{
	...
	{
		if(constant(strtoupper($ini['module'].'_'.$ini['name'])) == $ini['value']) continue;
		$up=1;
		$_db->sql('update '.T_REACTOR_CONFIG.' c,'.T_REACTOR_MODULE.' m set c.`value`="'.$ini['value'].'" where c.fk_module=m.pk_module and m.`name`="'.$ini['module'].'" and c.`name`="'.$ini['name'].'"');
	}
    if($up==0)return;
	include_once LIB_DIR.'config_write.php';
	configCompile();
...
function reactor_trace($msg)
{
	...
	if(isset($_SERVER['REQUEST_METHOD']))echo '<br>';
	}
}
...
$object=new reactor_interface($name_or_pool_id);
	...
	if(!isset($_obj[$action_name])){reactor_trace('undefined action'); return 0;}
...
if($tpl_module!='')initModule($tpl_module);
if($template!='')$Gekkon->display($template);
if($tpl_module!='')uninitModule();
...
{
	if(!isset($GLOBALS['_pool'][$_pool_id])) reactor_error('undefined _pool_id '.$_pool_id.' in pool_get');
	if($name=='')return $GLOBALS['_pool'][$_pool_id];
...
{
	session_start();
	if(isset($_SESSION['_user']))$_user=$_SESSION['_user'];
	$_SESSION['_user']=&$_user;

	if($_user['system']!=SITE_URL)$_RGET['logout']=1;
	if($_user['ip']!=$_SERVER['REMOTE_ADDR'])$_RGET['logout']=1;
}
else
{
...
{
	if(!isset($_POST['cookie']))$_POST['cookie']=0;
...
{
global $_RGET,$_SGET;
if(isset($_SGET[$name]))return $_SGET[$name];
...
function initRequestIndex()
{
	...
	$url = str_replace('//', '/', $url);
	$url = urldecode($url);
	if(substr($url,-1) == '/') $url = substr($url, 1, -1);
	if(substr($url,-5) == '.html') $url = substr($url, 1, -5);
...
function compileAurl($data)
{
	...
	if(!isset($_site_tree['param'][$show]))reactor_error('undefined show ['.$show.'] in site_tree');
	...
	foreach($data[$k] as $z => $a)
	{
		if(is_array($a))
		{
			...
			{
				if($w.''!='')
					$url.=rawurlencode($k).'%5B'.rawurlencode($z).'%5D%5B'.rawurlencode($t).'%5D='.rawurlencode($w).'&';
				else
