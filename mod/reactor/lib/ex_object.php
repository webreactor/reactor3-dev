...
if($where!='')$where_rez.=$where;
if($this->order!='')$where_rez.=' order by '.$this->order;

$data=$this->_db->pages('select * from '.$this->table.' where '.$where_rez,$page,$per_page,$all,$total_rows_count);
...
if($this->_db->sql('insert into '.$this->table.' (`'.implode('`, `',array_keys($data)).'`) values ("'.implode('", "',$data).'")'))
{
	$pk=array();
...
if($this->_db->sql('replace into '.$this->table.' (`'.implode('`, `',$rows).'`) values ("'.implode('", "',$rows).'")'))
return $pk;
else
return 0;
}
...
{
$this->_db->sql('select `'.$this->fkey.'` from '.$this->table.' where '.keys_implode($pk,$this->pkeys));
$t=$this->_db->line();
if($t==0)return 0;
}
$direct='-3';
if($order!=$this->order)
$direct='+3';
$this->_db->sql('update '.$this->table.' set '.$order.'='.$order.$direct.' where '.keys_implode($pk,$this->pkeys));
