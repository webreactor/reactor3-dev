...
class ex_object
{
    ...
    function getList($page, $per_page, $where = '')
    {
        ...
        $data = $this->_db->pages('select * from ' . $this->table . ' where ' . $where_rez, $page, $per_page, $all,
            $total_rows_count);
    ...
    function insert($data)
    {
        ...
        if ($this->_db->sql('insert into ' . $this->table . ' (`' . implode('`, `',
    ...
    function replace($data, $pk = 0)
    {
        ...
        if ($this->_db->sql('replace into ' . $this->table . ' (`' . implode('`, `',
                $rows) . '`) values ("' . implode('", "', $rows) . '")')
        ) {
            return $pk;
        } else {
            return 0;
    ...
    function moveUp($node_key)
    {
        ...
        if (count($this->fkeys) > 0) {
            $this->_db->sql('select `' . $this->fkey . '` from ' . $this->table . ' where ' . keys_implode($pk,
                    $this->pkeys));
            $t = $this->_db->line();
            if ($t == 0) {
                return 0;
        ...
        $this->_db->sql('update ' . $this->table . ' set ' . $order . '=' . $order . $direct . ' where ' . keys_implode($pk,
                $this->pkeys));
