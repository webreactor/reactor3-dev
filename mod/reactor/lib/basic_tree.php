...
$this->_db->sql('delete from '.$this->table.' where `'.$this->pkey.'` in ("'.implode('","',$keys).'")');
return $keys;
}
...
if($level > 0) $path = array_slice($path, 0, $level);
...
{	foreach($this->img as $nk=>$node)
	if($node[$this->fkey]==$node_key){$node_key=$nk;break;}
...
do
{
$this->_db->sql('update '.$this->table.' a, '.$this->table.' b set a.`'.$level.'`=b.`'.$level.'`+1 where a.`'.$this->fkey.'`=b.`'.$this->pkey.'` and a.`'.$level.'` =0 and b.`'.$level.'` >0');
}while($this->_db->affected_rows()>0);

if($childs=='')return 1;
$this->_db->sql('select max(`'.$level.'`) as maxlevel from '.$this->table);
$maxlevel=$this->_db->line();$maxlevel=$maxlevel['maxlevel'];

//first childs
$this->_db->sql('TRUNCATE TABLE `'.T_REACTOR_TREE_SUP.'`');
$this->_db->sql('insert into `'.T_REACTOR_TREE_SUP.'` select t.`'.$this->fkey.'` as pk_node,count(*) as childs from '.$this->table.' t group by t.`'.$this->fkey.'`');

$this->_db->sql('update '.$this->table.' a, `'.T_REACTOR_TREE_SUP.'` b set a.`'.$childs.'`=b.childs where a.`'.$this->pkey.'`=b.pk_node');

//all childs
for($i=$maxlevel;$i>1;$i--)
{
$this->_db->sql('TRUNCATE TABLE `'.T_REACTOR_TREE_SUP.'`');
$this->_db->sql('insert into '.T_REACTOR_TREE_SUP.' select t.`'.$this->fkey.'` as pk_node,count(*)+sum(t.`'.$childs.'`) as childs from '.$this->table.' t where t.`'.$level.'`='.$i.' group by t.`'.$this->fkey.'`');

$this->_db->sql('update '.$this->table.' a, `'.T_REACTOR_TREE_SUP.'` b set a.`'.$childs.'`=b.`'.$childs.'` where a.`'.$this->pkey.'`=b.`pk_node`');
}

if($k_left==''||$k_right=='')return 2;
//lefts rights
$this->_db->sql('set @a:=0');
$this->_db->sql('set @gr:=0');
$this->_db->sql('update '.$this->table.' set `'.$k_left.'`=@a+1,`'.$k_right.'`=(@a:=(`'.$k_left.'`+`'.$childs.'`*2+1)) where `'.$level.'`=1');
for($i=2;$i<=$maxlevel;$i++)
{
$this->_db->sql('update '.$this->table.' a,'.$this->table.' b  set a.`'.$k_left.'`=b.`'.$k_left.'`+1 where a.`'.$this->fkey.'`=b.`'.$this->pkey.'` and a.`'.$level.'`='.$i);
$this->_db->sql('update '.$this->table.' set `'.$k_right.'`=@a:=if(`'.$this->fkey.'`=@gr,@a+1,`'.$k_left.'`),`'.$k_right.'`=@gr:=`'.$this->fkey.'`,`'.$k_left.'`=@a,`'.$k_right.'`=@a:=`'.$k_left.'`+`'.$childs.'`*2+1 where `'.$level.'`='.$i.' order by `'.$this->fkey.'`');
}
