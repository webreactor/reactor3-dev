...
class retailrocket
{
	...
	function requestRemoteAPI($rocketFunct, $rocketValue = '', $paramArr = null)
	{
		...
		if (is_array($paramArr)) $getSendStr .='?'.http_build_query($paramArr);
		$getSendStr .= " HTTP/1.0\r\n";
		...
		$answerStr='';
		while (!feof($this->socket)) $answerStr .= fgets($this->socket);
		fclose($this->socket);
		$answerStr=explode("\r\n\r\n",$answerStr);
		if(stripos($answerStr[0],'chunked')!==false)$answerStr[1]=http_chunked_decode($answerStr[1]);
	...
	{
		if ($cookie=='') $cookie = $_COOKIE['rrpusid'];
