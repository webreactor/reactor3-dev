...
//$text=preg_replace('/([а-яА-Я])\.([а-яА-Я])\.\s+([а-яА-Яa-zA-Z0-9]+)/u', '\1.к.&nbsp;\2', $text);
// А. Б. Шеин
    $text = preg_replace('/([А-ЯA-Z]\.)\s*([А-ЯA-Z]\.)\s*([А-ЯA-Z][а-яa-z]+)/u', '\1&#x202f;\2&nbsp;\3', $text);
// А. Шеин
    $text = preg_replace('/([А-ЯA-Z]\.)\s*([А-ЯA-Z][а-яa-z]+)/u', '\1&#x202f;\2', $text);
// Шеин А. Б.
    $text = preg_replace('/([А-ЯA-Z][а-яa-z]+)\s*([А-ЯA-Z]\.)(\s*([А-ЯA-Z]\.))?/u', '\1&nbsp;\2&#x202f;\4', $text);
// Потому_что
    $text = preg_replace('/([а-яА-Я]+)\s+что/u', '\1&nbsp;что', $text);
...
//вынос
    ...
    if (substr($text, 0, 61) == '<d2JyIGNsYXNzPSJ0eXBvIg==><span class="sbrace typo"> </span> ') {
        $text = substr($text, 61);
    }
    if (substr($text, 0, 61) == '<d2JyIGNsYXNzPSJ0ebuXBvIg==><span class="sbdquo typo"> </span> ') {
        $text = substr($text, 61);
    }
    $text = preg_replace_callback('/<(\/?span[^>]*)>/su', 'typo_tag_encode', $text);
