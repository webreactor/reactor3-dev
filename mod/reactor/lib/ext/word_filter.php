 ...
 *
 * Согласно статье 20.1 КоАП РФ нецензурная брань в общественных местах (интернет — место общественное) расценивается
 * как мелкое хулиганство, за что установлена административная ответственность — наложение штрафа в размере от пятисот
...
class Text_Censure {
     ...
     *                                   в режиме $is_html === TRUE html код игнорируется, а html сущности заменяются в
     *                                   "чистый" UTF-8
     * @param    string|null $replace    строка, на которую заменять матный фрагмент, например: '[ой]' ($replace д.б. в
     *                                   кодировке $charset) опция работает в PHP >= 5.2.0
     * @param    string      $charset    кодировка символов (родная кодировка -- UTF-8, для других будет прозрачное
     *                                   перекодирование)
     *
     * @return   bool|string|int|null    Если $replace === NULL, то возвращает FALSE, если мат не обнаружен, иначе
     *                                   фрагмент текста с матерным словом. Если $replace !== NULL, то возвращает
     *                                   исходную строку, где фрагменты мата заменены на $replace. В случае
     *                                   возникновения ошибки возвращает код ошибки > 0 (integer):
     *                                     * PREG_INTERNAL_ERROR
    ...
    public static function parse(
        ...
        if ($re_badwords === null) {
            ...
            $pretext = array(
                ...
                '[нn]_?[еe]_?[вv]_?[ьъ]_? (?=[еёe])', #невъ (невъебенный)
                'ё_?             (?=[бb6])',          #ё      (ёбля)
                #2
                '[вvb]_?[ыi]_?',      #вы
                '[зz3]_?[аa]_?',      #за
		...
		*/
        ...
        if ($is_html) {
            ...
            $s = /*UTF8::*/
                html_entity_decode($s/*, $is_htmlspecialchars = true*/);
        }
        /*
