...
function content_adapter($_name_pool_so='')
{	global $_interfaces;	$this->define=array();
	...
	if($_name_pool_so!='')
	{
		if(isset($_interfaces[$_name_pool_so]))	{ $this->configure($_interfaces[$_name_pool_so]['define']); return; }
		if(isset($_SESSION['_stored_interface'][$_name_pool_so])) { $this->restore($_SESSION['_stored_interface'][$_name_pool_so]['define']); return; }
...
{	$this->_drop=$fields;
	if($fields=='none')$this->_drop=array();}

function take($fields='none')
{
	$this->_take=$fields;
	if($fields=='none')$this->_take=array();
}
