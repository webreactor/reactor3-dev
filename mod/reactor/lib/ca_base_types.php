...
class ca_text extends base_type{
	function fromForm(&$value)
	{
		...
		if(!isset($prop['typo']))$prop['typo']=array();
		if(!isset($prop['no-typo']))$value=typo($value,$prop['typo']);
		$value=htmlspecialchars($value,ENT_QUOTES);
		if(!isset($prop['no-br'])) $value=str_replace(array("\r","\n"),array('','<br />'),$value);
	...
	if(!isset($prop['no-links'])) $value=preg_replace('/<a href="([^"]+)">[^<]+<\/a>/u','\1',$value);
...
class ca_email extends base_type
{
	function fromForm(&$value)
	{
		...
		if($value=='')
		{
			...
			if($this->get('necessary')==1)return 0;
			return 1;
		}
