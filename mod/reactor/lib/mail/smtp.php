...
class smtp extends socket_client
{
	...
	var $letter;
	var $text;
	var $header_fields = array('from','date','to','cc','return-path',
		'subject','mime-version','content-type',
		'content-transfer-encoding','reply-to','sender','content-disposition',
		'content-id');

	function smtp($srv='',$login='',$pwd='')
	{
		if($srv!='')$this->smtp_connect($srv,$login,$pwd);
	}
	...
	function smtp_connect($srv,$login,$pwd)
	{
		...
		if($this->get_answ()!='250')
			$this->error = 3; //Error 3: Wrong HELO request.
		else
		{
			$this->put('AUTH LOGIN');
			if($this->get_answ()=='334')
			{
				...
				if($this->get_answ()!='235') $this->error = 5; //Error 5: Wrong login\password.
			}
		}
		$this->error = 0;
	}
	...
	{
		if(!is_file($fname))$this->error = 10; //Error 10: Can't open file.
		$fd = fopen($fname, 'r');
		...
		$this->letter['data'][]=array('header'=>
			array('content-type'=>$type.'; name="'.$fname.'"',
	...
	function send($to)
	{
		...
		if($this->get_answ()=='250')
		{
			...
			if($this->get_answ()!='250') $this->error = 12; //Error 12: Wrong field 'FROM' in header.
			$this->put('RCPT TO: '.$to);
			if($this->get_answ()=='250')
			{
				$this->put('DATA');
				...
				$this->error = 14; //Error 14: Unknown recipent or user is not local.
		}else{
	...
	function quit()
	{
		...
		if($this->get_answ()!='221')
			$this->error = 20; //Error 20: Can't QUIT right now.
		$this->close();
	}
	...
	function combine_head(&$header)
	{
		...
		if($type=='text')
		{
			...
			$header['content-type']=preg_replace('/charset="?([^;"]+)"?/is','charset="windows-1251"',$header['content-type']);
			else
		...
		$header['content-type']=preg_replace_callback('/(name)="?([^;"]+)"?/is','encode_field_t',$header['content-type']);

		if(isset($header['content-disposition']))
			$header['content-disposition']=preg_replace_callback('/(filename)="?([^;"]+)"?/is','encode_field_t',$header['content-disposition']);
	...
	{
		$this->text ='';
		if(!isset($data['header']['from'])/* or !isset($data['header']['to'])*/)return 0;

		if(!isset($data['header']['date'])) $data['header']['date']=date("r");
		if(!isset($data['header']['mime-version'])) $data['header']['mime-version']="1.0";
		if(!isset($data['header']['content-type'])) $data['header']['content-type']="text/plain";
