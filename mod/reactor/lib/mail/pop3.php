...
function pop3($srv,$login,$pwd)
{
	...
	if($this->get_answ()!='+OK')
		$this->error = 3; //Error 3: Wrong server reply on connect.
	else
	{
		$this->put('AUTH LOGIN');
		if($this->get_answ()=='+')
		{
			...
			if($this->get_answ()!='+OK') $this->error = 5; //Error 5: Wrong login\password.
		}else{			$this->put('USER '.$login);
			$this->get();
			$this->put('PASS '.$pwd);
			if($this->get_answ()!='+OK') $this->error = 5; //Error 5: Wrong login\password.		}
...
$data['header']['c/t-content']=substr($data['header']['content-type'],0,$t=strpos($data['header']['content-type'],'/'));
$data['header']['c/t-type']=substr($data['header']['content-type'],$t+=1,$t=strpos($data['header']['content-type'].';',';')-$t);
