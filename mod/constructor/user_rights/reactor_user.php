...
class reactor_user extends basic_object
{
	function getList($page,$per_page,$filter='',$fk_ugroup=0)
	{
		...
		$all=0;
		$where_rez=' where 1 ';
		if($filter!='')$where_rez.=' and login like "%'.addslashes($filter).'%" ';
		if($fk_ugroup!=0)$where_rez.=' and fk_ugroup='.$fk_ugroup.' ';
		$data=$this->_db->pagess('select * from '.T_REACTOR_USER.' '.$where_rez.' order by login',$page,$per_page,$all,$total_rows_count);
