...
$_db->sql('select * from `'.T_REACTOR_INTERFACE_ACTION.'` where fk_interface in('.implode(',',$keys).') order by fk_action');
...
foreach($module['table']['item'] as $item)
{
	if($item['mlng']==1)
	{
		foreach($_languages as $lng=>$v)
		{
			...
			if($t!=0)
			{
			$module['table']['create'][$item['db_name'].'_'.$lng]=preg_replace('/AUTO_INCREMENT=[\w]+/i','',$t['Create Table']);
			}
			if($item['rmp_data']==1)
			{
	...
	else
	{
		...
		$t=$_db->line();
		$module['table']['create'][$item['db_name']]=preg_replace('/AUTO_INCREMENT=[\w]+/i','',$t['Create Table']);
		if($item['rmp_data']==1)
		{
