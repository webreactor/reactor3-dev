...
function update_module($pk_module)
{
    ...
    $_db->sql('delete from `' . T_REACTOR_INTERFACE_DEFINE . '` where fk_interface in(' . implode(',',
            $interfaces) . ')');
    $_db->sql('delete from `' . T_REACTOR_INTERFACE_ACTION . '` where fk_interface in(' . implode(',',
            $interfaces) . ')');
