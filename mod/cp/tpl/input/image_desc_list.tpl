...
<ul class="files-desc-list clearfix" >
    ...
    <li class="img-drag">
        ...
        <div class="desc-bottom">
            <label>
                <input type = "text" value = "{$it.desc}" class = "wild_text">
            </label>
        </div>
    </li>
    <!--/foreach-->
    ...
    $(function () {
        ...
        $('#form_id_{$_form_id}').on('submit', function (ev) {
            ...
            $('.img-drag', this).each(function () {
                ...
                var obj = {'file': srcpath, 'real': srcpath, 'desc': $this.find('label>input').val()};
                if (!$this.is(":visible")) {
                    obj.delete = 1;
                }
                files.push(obj);
        ...
        $('.fileupload-buttonbar .accept').on('click', function (ev) {
            $('.template-download').each(function () {
                ...
                thumb.find('div').append(img).parent().append('<div class="desc-bottom"><label><input type = "text" class = "wild_text"></label></div>');
