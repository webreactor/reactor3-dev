...
function sel_int(id_input,id_text,data,param,callback)
{
	...
	function area_show(id)
	{
		...
		_this.obj.appendChild(_this.obj_search=_rt().create('input',{name:'_search',style:{width:'200px',margin:'0 20px'},value:'Начните поиск',onkeyup:handle_list_search,onclick:handle_list_search_start}));
		_this.obj.appendChild(_this.obj_loading=_rt().create('div',{style:{color:"#555555",display:'none'},className:'popUpMenuLine',innerHTML:'Загрузка...'}));
		_this.obj.appendChild(_this.obj_list=_rt().create('div',{innerHTML:'</br>'}));
		_this.obj_list.appendChild(_rt().create('a',{'href':'javascript:void(0)','innerHTML':'Сбросить значение','key':'','keyName':'Не выбрано'}));
		shadowing(_this.obj);
	}
	...
	function handle_list_search()
	{
		...
		time_f=now;
		if(time_h!=null){clearTimeout(time_h);time_h=null;}
	...
	function handle_search_answer(get,data)
	{
		...
		{
			f=1;
			_this.obj_list.appendChild(_rt().create('a',{'href':'javascript:void(0)','innerHTML':data[i],'key':i,'keyName':data[i]}));
		}
