...
if(!isset($_obj_data['action'][$action_name])){$Gekkon->display('login.tpl');die();}
...
if($_action['handler']>0)
{
	...
	{
		if($_action['cptpl']!='')
    	header('Location: '.SITE_URL.$_reactor['language_url'].'cp/'.$_action['cptpl'].'/'.$_action['cptpl_mod'].'/?'.$configurators);
    	else
	...
	if($_action['cptpl']!='')
	header('Location: '.SITE_URL.$_reactor['language_url'].'cp/'.$_action['cptpl'].'/'.$_action['cptpl_mod'].'/?_so='.$stored_id.'&'.$configurators);
	else
	header('Location: '.$_SERVER['HTTP_REFERER']);
    die();
	}
