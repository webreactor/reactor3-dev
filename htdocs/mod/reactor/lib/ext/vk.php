...
function vk_uploadImage($group_id, $token, $file, $filename = 'image')
{
    ...
    if ($hUpload) {
        ...
        $sResp = explode("\r\n\r\n", $sResp);
        if (stripos($sResp[0], 'chunked') !== false) {
            $sResp[1] = http_chunked_decode($sResp[1]);
        }
...
function vk_post($group_id, $token, $text, $images = array())
{
    ...
    $sReq  = 'https://api.vk.com/method/wall.post?owner_id=-' . $group_id . '&message=' . urlencode(
            $text
        ) . '&attachments=' . $attachments . '&access_token=' . $token;
    $oResp = json_decode(file_get_contents($sReq));
