...
//registration variable for using
    ...
    function clear_cache($template_name, $stream = '')
    {
        ...
        if ($stream != '') {
            ...
            $file = $compile_path . $cid[1] . $cid[2] . '/' . $cid;
        ...
        if (!is_dir($compile_path)) {
            ...
            $this->clear_cache_r($compile_path);
        }
    }
    ...
    function clear_cache_r($path)
    {
        if ($dh = opendir($path)) {
            while (($file = readdir($dh)) !== false) {
                if ($file[0] != '.') {
                    if (is_dir($path . $file)) {
                        Gekkon::clear_cache_r($path . $file . '/');
                        rmdir($path . $file);
                    } else {
                        unlink($path . $file);
                    }
    ...
    function compile_file($file_name, $virt_name = '')
    {
        ...
        require_once $this->gekkon_path . 'config.php';
    ...
    function compile_r(&$str)
    {
        ...
        if (!preg_match('/<!--\s*([^\s]+)\b(.*)-->/sU', $str, $arr)) {
            return $str;
        }
...
//die('Compile error: Dont know tag - '.$tag['name']);
    ...
    function compile_tag($tag)
    {
        $bin_open  = '';
        $bin_close = '';
        require $this->gekkon_path . 'plugin/' . $tag['name'] . '.php';
        if ($tag['compile'] == 1) {
