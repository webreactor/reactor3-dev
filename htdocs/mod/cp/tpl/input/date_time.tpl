...
<input name="{$item.name}" id="{$item.name}_{$_form_id}_{$_input_id}" title="{$item.description}" type="text" value="{$data.$item&name}" style="width:130px;" <!--if $item.base_type_param.input.disabled.isset()-->disabled<!--/if-->>
...
Calendar.setup({
	...
	ifFormat     : "%d.%m.%Y 0:0:0",
	daFormat     : "%d.%m.%Y 0:0:0"
});
</script>
<!--/if-->
<!--/box-->