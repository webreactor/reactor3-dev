...
//version 2.0.1
require_once LIB_DIR . 'reactor_ver.php';
function create_rmp($mod_name)
{
    ...
    $_db->sql(
        'select * from `' . T_REACTOR_INTERFACE_ACTION . '` where fk_interface in(' . implode(
            ',',
            $keys
        ) . ') order by fk_action'
    );
    ...
    foreach ($module['table']['item'] as $item) {
        if ($item['mlng'] == 1) {
            foreach ($_languages as $lng => $v) {
                ...
                $t = $_db->line();
                if ($t != 0) {
                    $module['table']['create'][$item['db_name'] . '_' . $lng] = preg_replace(
                        '/AUTO_INCREMENT=[\w]+/i',
                        '',
                        $t['Create Table']
                    );
            ...
            $t                                           = $_db->line();
            $module['table']['create'][$item['db_name']] = preg_replace(
                '/AUTO_INCREMENT=[\w]+/i',
                '',
                $t['Create Table']
            );
