...
function update_module($pk_module)
{
    ...
    $_db->sql(
        'SELECT a.pk_action,concat(i.name ,">",a.name) as act
    ...
    $_db->sql(
        'SELECT m.pk_action,m.name as fm_action,i.name as fm_interface FROM `' . T_REACTOR_INTERFACE_ACTION . '` m, `' . T_REACTOR_INTERFACE . '` i where m.fk_interface=i.pk_interface'
    );
    $named_actions = array();
    while ($t = $_db->line()) {
